...
from gamenight.games.splendor.gui import _seat_layout, draw_board, status_text

BG = "#0f0c09"
CARD_BG = "#1c1712"
CARD_BORDER = "#3a2f22"
TEXT_COLOR = "#f3ece0"
DIM_TEXT_COLOR = "#a8998a"
WINNER_COLOR = "#7CFFB2"


class BracketRevealViewer:
    """Single-window bracket "reveal" for Splendor -- same shape as
    `battleship/bracket_gui.py`'s `BracketRevealViewer` (bracket tree, a
    "Play Next Match" button, an embedded board that replays each match's saved
    games), adapted for a single 2-player board instead of two side-by-side ones,
    since every round-robin/bracket matchup in this repo is a 2-player Splendor game
    regardless of how many total players Splendor itself supports (see
    `../README.md`'s "Player Count" section for why matches stay pairwise even
    though `SplendorGame` supports up to 4).
    ...
    def __init__(
        ...
        # Every round-robin/bracket matchup is a 2-player Splendor game (see the
        # class docstring) -- reuse the same seat layout the live GUI uses at 2
        # players, just for a single embedded board rather than a full table.
        self.player_ids = ["player_1", "player_2"]
        self.layout = _seat_layout(2)
        ...
        board_width = self.layout["canvas_w"] + 140 + standings_width
        self.root.geometry(f"{max(int(board_width), 900)}x1080")
        ...
        # taller than the window (many bracket rounds, or Splendor's tall board)
        # without cutting off the controls at the bottom.
    ...
    def _build_board(self, parent: tk.Frame) -> None:
        ...
        tk.Label(
            ...
            wraplength=int(self.layout["canvas_w"]),
        ).pack(pady=(6, 6))

        self.matchup_var = tk.StringVar(value="")
        tk.Label(
            parent, textvariable=self.matchup_var, font=("Helvetica", 11, "bold"), fg=DIM_TEXT_COLOR, bg=BG
        ).pack(pady=(0, 6))
        ...
        self.canvas = tk.Canvas(
            board_frame,
            width=self.layout["canvas_w"],
            height=self.layout["canvas_h"],
            bg="#241a12",
    ...
    def _reveal_all(self) -> None:
        ...
        if final_series is not None and final_series["games"]:
            self.matchup_var.set(f"{final_series['bot_a']}  vs  {final_series['bot_b']}")
            ...
            if events:
                self._update_board(events[-1]["state"], swap)
    ...
    def _play_match(self, round_idx: int, match_idx: int, series: dict[str, Any]) -> None:
        ...
        self.matchup_var.set(f"{series['bot_a']} (bottom)  vs  {series['bot_b']} (top)")

        for game_index, game in enumerate(series["games"]):
            if self.closed:
                return
            delay = self.first_game_delay if game_index == 0 else self.rest_delay
            # Keep bot_a in the bottom seat and bot_b in the top seat across every
            # game of this match, regardless of which player_id (player_1/player_2)
            # each bot was actually assigned that game (starting player alternates).
            ...
            for event in events:
                ...
                self._update_board(event["state"], swap)
    ...
    def _update_board(self, state: dict[str, Any], swap: bool) -> None:
        """Draws the board so `series["bot_a"]` always renders in the bottom seat
        and `series["bot_b"]` in the top seat, regardless of which player_id
        (player_1/player_2) each bot was assigned this particular game -- done by
        simply reordering which player_id `draw_board` looks up for each seat
        index, not by touching the state itself.
        """
        seat_order = list(reversed(self.player_ids)) if swap else self.player_ids
        draw_board(self.canvas, state, seat_order, self.layout)
